...
def deleteData():
    if not tree.selection():
        msgb.showwarning("Nenhum item selecionado.", "Selecione o item que deseja deletar!", icon="warning")
    else:
        ask = msgb.askquestion("Tem certeza?", "Tem certeza que deseja deletar esse item? ", icon="warning")
        if ask == 'yes':            
...
def onSelected(event):
    ...
    lblWarning = Label(registerWarning, text="Caso não tenha feito a avaliação,por favor deixe como zero!", font=("consolas", 10), bg="#d1ad5e", width=250)
    lblTitle = Label(registerTitle, text="Atualize os dados",font=("consolas", 25), bg="#7cbb63", width=300)
    lblTitle.pack(fill=X)
    lblsubject = Label(registerContact, text="Insira a materia", font=("consolas", 10))
    lblsubject.grid(row=0, sticky=W)
    lblAV1 = Label(registerContact, text="Insira a sua nota da AV1",font=("consolas", 10))
    lblAV1.grid(row=1, sticky=W)
    lblAV2 = Label(registerContact, text="Insira a sua nota da AV2",font=("consolas", 10))
    lblAV2.grid(row=2, sticky=W)
    lblAV3 = Label(registerContact, text="Insira a sua nota da AV3",font=("consolas", 10))
    lblAV3.grid(row=3, sticky=W)
    lblAVD = Label(registerContact, text="Insira a nota da AVD",font=("consolas", 10))
    lblAVD.grid(row=4, sticky=W)
    lblAVDS = Label(registerContact,text="Insira a nota da AVDS", font=("consolas", 10))
...
def submitData():
    if AV1.get() == None or subject.get() == "" or AV2.get() == None or AV3.get() == None or AVD.get() == None or AVDS.get() == None:
        resultado = msgb.showwarning("Campo vazio!", "Você esqueceu de preencher algum campo. Verifique os campos!", icon="warning")
    else:
        tree.delete(*tree.get_children())
        nota1 = 0.0
        nota2 = 0.0
...
def insertData():
    ...
    registerWarning.pack(side=BOTTOM)
    lblWarning = Label(registerWarning, text="Se não tiver feito a avaliação, por favor deixe como zero!", font=("consolas", 12), bg="#d1ad5e", width=200)
    ...
    lblAV3 = Label(registerContact, text="Insira a sua nota da AV3",font=("consolas", 10))
    lblAV3.grid(row=3, sticky=W)
    lblAVD = Label(registerContact, text="Insira a sua nota da AVD",font=("consolas", 10))
    lblAVD.grid(row=4, sticky=W)
    lblAVDS = Label(registerContact, text="Insira a sua nota da AVDS",font=("consolas", 10))
    lblAVDS.grid(row=5, sticky=W)
    subjectEntry = Entry(registerContact, textvariable=subject, font=('arial', 12))
    subjectEntry.grid(row=0, column=1)
    AV1Entry = Entry(registerContact, textvariable=AV1, font=('arial', 12))
