...
def updateData():
    ...
    if(average.get()>=6):
        result = ' PARABÉNS, VOCÊ ESTÁ APROVADO!'
    else:
...
def onSelected(event):
    ...
    lblWarning = Label(registerWarning, text="Caso não tenha realizado a avaliação,por favor deixe como zero!", font=("consolas", 10), bg="#d1ad5e", width=250)
...
def submitData():
    if AV1.get() == None or subject.get() == "" or AV2.get() == None or AV3.get() == None or AVD.get() == None or AVDS.get() == None:
        resultado = msgb.showwarning("Você esqueceu de preencher algum campo. Verifique os campos!", icon="warning")
    else:
        tree.delete(*tree.get_children())
        nota1 = 0.0
        nota2 = 0.0
